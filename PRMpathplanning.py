...
def connect_nodes(nodes, k, inflated_obstacle_map):
    for node in nodes:
        ...
        for other_node in nodes:
            if node != other_node and not line_in_obstacle(
                node.x, node.y, other_node.x, other_node.y, inflated_obstacle_map
            ):
...
# Fixed start and goal coordinates (change these to your desired positions)
start_x, start_y = 1250, 400
goal_x, goal_y = 100, 350
...
# Create start and goal nodes with fixed positions
start = Node(start_x, start_y)
goal = Node(goal_x, goal_y)

# Generate random nodes and add start/goal to the network
nodes = generate_random_nodes(num_nodes, x_max, y_max, inflated_obstacle_map)
nodes.insert(0, start)  # Add start at beginning
nodes.append(goal)  # Add goal at end
